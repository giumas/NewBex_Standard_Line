...
%                 end
                ...
                idx = find(filter2(B,TargetSizeMask) < 20);
                TargetSizeMask(idx) = 0;
                
                % now cluster the targets along each beam - 1 target per
                % cluster
                gap = 15;       % this is in samples
                TargetSizeMask2 = zeros(size(TargetSizeMask));
                for i = 1:length(TS(:,1))
                    idx = find(TargetSizeMask(i,:) == 1);
                    L = cluster1D(idx,gap);
                    % pick the center of the sample
                    for i2 = 1:max(L)
                        idx2 = find(L == i2);
                        if length(idx2) > 0
                            idxKeep = round(mean(idx(idx2)));
                            TargetSizeMask2(i,idxKeep) = 1;
                        end
                    end
                end
                ...
                if plott
                    ...
                    a(6) = subplot(166);
                    imagesc( TS' + BottomMask' + (1-TargetSizeMask2')*(-999)); caxis([-80 -20]);
                    title('clustered along beams')                    
                ...
                TSfinal = (TS + BottomMask + (1-TargetSizeMask2)*(-999));
